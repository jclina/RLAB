...
A basic **if statement** will evaluate a some expression is true or false and then executes a a specified task if the expression is true.
...
And here is an example of `case_when()`. Here, you are evaluating multiple inputs for multiple conditions. 
```{r}
#| echo: true
...
library(tictoc)
n_people <- 1000000
daily_avg_steps <- sample(3000:12000, n_people, replace = TRUE)

tic("Without preallocation")
steps_no_prealloc <- list()
for (i in seq_along(daily_avg_steps)) {
  steps_no_prealloc[[i]] <- rpois(7, lambda = daily_avg_steps[i])
}
toc()

tic("With preallocation")
steps_prealloc <- vector("list", length(daily_avg_steps))
for (i in seq_along(daily_avg_steps)) {
  steps_prealloc[[i]] <- rpois(7, lambda = daily_avg_steps[i])
}
toc()

# Without preallocation: 1.67 sec elapsed 
# With preallocation: 1.06 sec elapsed 
```
